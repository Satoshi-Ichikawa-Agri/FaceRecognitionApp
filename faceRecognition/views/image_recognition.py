...
def image_save(processing_upload_file, original_file):
    ...
    """
    # 完全パスの作成
    original_file_path = os.path.join(IMAGE_DIR + str(original_file))

    # DB登録
    create_image = UploadImages.objects.create(image_name=original_file_path)

    # 画像をディレクトリに保存
    cv2.imwrite(original_file_path, processing_upload_file)

    return original_file_path, create_image
...
def detect(image_file, cascade_file_name, original_file_path, create_image):
    ...
    # DBに登録する
    DetectResult.objects.create(upload_image=create_image, count=number_of_individuals)
